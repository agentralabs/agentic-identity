...
fn main() {
    ...
    println!("  ID:         {id}");
    println!("  Name:       {}", anchor.name.as_deref().unwrap_or("(none)"));
    ...
    doc.verify_signature().expect("document signature should verify");
    println!("  Verified:   OK");
    println!();
    ...
    let rotated = anchor.rotate(RotationReason::Scheduled).expect("rotation should succeed");
    println!("Key rotated (reason: scheduled)");
    println!("  New ID:       {}", rotated.id());
    println!("  New pub key:  {}", rotated.public_key_base64());
    println!("  Rotation history: {} entries", rotated.rotation_history.len());
    println!(
    ...
    let rotated2 = rotated.rotate(RotationReason::Manual).expect("second rotation");
    println!("  After second rotation: {} history entries", rotated2.rotation_history.len());
    println!();
