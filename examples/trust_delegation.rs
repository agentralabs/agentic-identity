...
use agentic_identity::trust::chain::{verify_trust_chain, validate_delegation};
...
fn main() {
    ...
    let grant_ab = TrustGrantBuilder::new(
        alice.id(),
        bob.id(),
    ...
    println!("  Capabilities:  {:?}", grant_ab.capabilities.iter().map(|c| &c.uri).collect::<Vec<_>>());
    ...
    let grant_wildcard = TrustGrantBuilder::new(
        alice.id(),
        bob.id(),
        bob.public_key_base64(),
    )
    ...
    let checks = ["read:calendar", "read:email", "read:files", "write:calendar"];
    for cap in &checks {
    ...
    let constrained_grant = TrustGrantBuilder::new(
        alice.id(),
        bob.id(),
        bob.public_key_base64(),
    )
    ...
    .constraints(
        TrustConstraints::time_bounded(now, now + one_hour)
            .with_max_uses(5),
    )
    .sign(alice.signing_key())
    ...
    let grant_delegatable = TrustGrantBuilder::new(
        alice.id(),
        bob.id(),
        bob.public_key_base64(),
    )
    ...
    println!("  Delegation: allowed={}, max_depth={:?}",
        grant_delegatable.delegation_allowed,
        grant_delegatable.max_delegation_depth
    );
    println!();

    // Validate that Bob CAN delegate to Carol.
    let delegation_caps = vec![Capability::new("read:calendar")];
    validate_delegation(&grant_delegatable, &delegation_caps)
    ...
    let grant_bc = TrustGrantBuilder::new(
        bob.id(),
        carol.id(),
        carol.public_key_base64(),
    )
    ...
    let chain_write = verify_trust_chain(&chain, "write:calendar", &[])
        .expect("verification should succeed");
    ...
    let mut ack_grant = TrustGrantBuilder::new(
        alice.id(),
        bob.id(),
        bob.public_key_base64(),
    )
    ...
    ack_grant.acknowledge(bob.signing_key()).expect("acknowledgment");
    println!("  After:  {}...", &ack_grant.grantee_acknowledgment.as_ref().unwrap()[..32]);
    println!();
    ...
    let revocable_grant = TrustGrantBuilder::new(
        alice.id(),
        bob.id(),
        bob.public_key_base64(),
    )
    ...
    let v_after = verify_trust_grant(&revocable_grant, "read:calendar", 0, &[revocation.clone()]).unwrap();
    ...
    let chain_revoked = verify_trust_chain(
        &chain_with_revocation,
        "read:calendar",
        &[revoke_root],
    )
